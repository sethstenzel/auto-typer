import time
from auto_typer.classes.data import SingleKey, MultiKeys, TimedPause, MouseScroll
...
import pythoncom
import flet as ft
from auto_typer.classes.components import AppCoverAlert
        ...
        while not token_completed:
            if isinstance(token, MultiKeys):
                ...
                if self.check_window_focused(pause_if_not=True):
                    ...
                    if (token.key == "enter" and self.app_state.pause_on_new_line) or (token.key == "enter" and self.app_state.advance_to_newline > 0):
                        if self.app_state.advance_to_newline > 0:
                            self.app_state.advance_to_newline -= 1
                        self.paused = True
                ...
                for char in token:
                    ...
                    while not char_completed:
                        if (
                            ...
                            (self.play and self.paused and self.app_state.advance_to_newline > 0) or
                            (self.play and self.paused and self.app_state.advance_token > 0)
                        ):  
                            self.focus_window()
                            kb.press(char)
                            time.sleep(self.speed/1000)
            ...
            while not token_completed and self.play: 
                if (
                    ...
                    if (not self.paused and self.check_window_focused(pause_if_not=True) or
                        ...
                        if token_completed and self.app_state.advance_token > 0:
                            self.app_state.advance_token -= 1
                if self.paused:
                    time.sleep(0.1)
            if not self.play:
                break
